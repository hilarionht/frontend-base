...
export class AuthService {
  ...
  async login(credentials: LoginCredentials): Promise<LoginResult> {
    try {
      const res = await firstValueFrom(
        this.http
          .post<ApiLoginResponse>(`${this.apiBaseUrl}/api/auth/login`, credentials)
  ...
  async requestPasswordRecovery(email: string): Promise<void> {
    await firstValueFrom(
      this.http
        .post(`${this.apiBaseUrl}/api/auth/recovery`, { email })
        .pipe(timeout(HTTP_TIMEOUT_MS)),
    );
  }
  ...
  refreshToken(): Observable<boolean> {
    ...
    this.refreshInFlight$ = this.http
      .post<ApiLoginResponse>(`${this.apiBaseUrl}/api/auth/refresh`, {})
      .pipe(
  ...
  private setupStorageListener(): void {
    if (!this.isBrowser || this.storageListenerRegistered) return;
